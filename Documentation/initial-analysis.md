...
Python will be used to fetch financial data and pack it into a CSV file that feeds the C++ predictive analysis engine.
...
* Pack stock data into a CSV file to serve the C++ modelling engine
...
yfinance.ticker will allow me to fetch information about a singular stock.
```python
ticker.history(period="5d", interval="1d") # fetch the history of a ticker in the given time range
```

### Pandas
Pandas is a fast, open-source data manipulation python module that I will use to build up my backend via formatting the yfinance data into a useable format for the C++ side.
```python
...
df.to_csv("data.csv", index=False) # converts pandas dataframe to readable csv
```
...
3. pack into csv file to send to c++ file
...
### C++
1. Read CSV file generated by python file
2. Transform CSV contents into native C++ `std::vector`
